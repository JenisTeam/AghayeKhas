...
local function run(msg, matches)
    if matches[1] == 'پاکسازی' and is_mod(msg) then
        if tostring(msg.to.id):match("^-100") then 
            if tonumber(matches[2]) > 1000 or tonumber(matches[2]) < 1 then
                return  '`••اخطار\n➖➖➖➖➖➖➖➖➖➖➖\n••تعداد پیام های قابل پاک سازی در هر دفعه [ 1 - 1000 ] می باشد 💠`'
            else
                tdcli.getChatHistory(msg.to.id, msg.id,0 , 100, delmsg, {msgs=matches[2]})
                return "••انجام شد\n➖➖➖➖➖➖➖➖➖➖➖\n[ `"..matches[2].. "` ] پیام قبلی گروه با موفقیت پاکسازی شد 💠"
            end
        else
            return '•• اخطار...\n➖➖➖➖➖➖➖➖➖➖➖\n•• این قابلیت فقط در سوپرگروه ممکن است 💠'
        end
    end
end
return {
